...
/**
 ...
 - 4 feature maps of different size. Shapes : (#channels,256,256), ($channels,128,128),
...
@objc(PyramidROIAlignLayer) class PyramidROIAlignLayer: NSObject, MLCustomLayer {
    ...
    func evaluate(inputs: [MLMultiArray], outputs: [MLMultiArray]) throws {
        ...
        os_signpost(OSSignpostType.begin, log: log, name: "PyramidROIAlign-CreateInputResources")
        
        let featureMapImages = featureMaps.map { (featureMap) -> MPSImage in
        ...
        os_signpost(OSSignpostType.end, log: log, name: "PyramidROIAlign-CreateInputResources")
        ...
        os_signpost(OSSignpostType.begin, log: log, name: "PyramidROIAlign-CreateOutputResources")
        ...
        os_signpost(OSSignpostType.end, log: log, name: "PyramidROIAlign-CreateOutputResources")
        ...
        for (i,batch) in batches.enumerated() {
            ...
            performBatch(commandQueue: commandQueue,
                         ...
                         batch: batch,
                         outputImages: outputImagesByBuffers[outputImagesIndex]) { outputItems in
    ...
    for group in batch.groups {
        ...
        case .regions(featureMapIndex: let featureMapIndex, regions: let regions):
            ...
            for (r, region) in regions.enumerated() {
                ...
                let kernel = MPSNNCropAndResizeBilinear(device: commandQueue.device, resizeWidth: outputWidth, resizeHeight: outputHeight, numberOfRegions: 1, regions: regionsPointer)
                for slice in 0 ..< channels/4 {
                    kernel.sourceFeatureChannelOffset = slice*4
                    kernel.destinationFeatureChannelOffset = slice*4
                    kernel.encode(commandBuffer: buffer!, sourceImage: inputImage, destinationImage: outputImage)
                }
...
func copyOutput(items: [ROIAlignOutputItem],
    ...
    for output in items {
        ...
        case .image(image: let image):
            image.readBytes(pointer, dataLayout: MPSDataLayout.featureChannelsxHeightxWidth, bytesPerRow: outputWidth*MemoryLayout<Float>.size,
...
@available(iOS 12.0, macOS 10.14, *)
func roisToInputItems(rois: MLMultiArray, featureMapSelectionFactor: CGFloat, imageSize: CGSize) -> [ROIAlignInputItem] {
    ...
    for i in 0..<totalCount {
        ...
        let region = MPSRegion(origin: MPSOrigin(x: x1, y: y1, z: 0), size: MPSSize(width: width, height: height, depth: 1.0))
        
        let content: ROIAlignInputItem.Content = { () -> ROIAlignInputItem.Content in
            if (regionIsValid) {
...
func groupInputItemsByContent(items: [ROIAlignInputItem], maxComputeBatchSize: Int) -> [ROIAlignInputGroup] {
    ...
    let closeRegionsGroupIfNecessary = { () -> Void in
    ...
    let closePaddingGroupIfNecessary = { () -> Void in
