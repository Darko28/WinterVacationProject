...
class MultiArrayBatchProvider: MLBatchProvider {
    ...
    init(multiArrays: [MLMultiArray], removeZeros: Bool, featureNames: [String]) {
...
class MultiArrayFeatureProvider: MLFeatureProvider {
    ...
    init(multiArrays: [MLMultiArray], featureNames: [String], index: Int) {
        self.multiArrays = multiArrays
    ...
    func featureValue(for featureName: String) -> MLFeatureValue? {
        ...
        guard let featureIndex = self.orderedFeatureNames.firstIndex(of: featureName) else { return nil }
...
func maximumValueWithIndex(values: [Float]) -> (Float, UInt) {
    ...
    vDSP_maxvi(UnsafeMutablePointer<Float>(&values), 1, resultValuePointer, resultIndexPointer, vDSP_Length(values.count))
