...
/**
 ...
 Regions that overlap more than a given threshold are removed through
 a process called Non-Max Supression (NMS).
 
 The regions of interest are adjusted using deltas provided as input to refine
 how they enclose the detected objects.
 
 The layer taks three inputs:
 ...
 The class probability input is layed out such that the index 0 corresponds to the background class.
...
@objc(DetectionLayer) class DetectionLayer: NSObject, MLCustomLayer {
    ...
    // Maxmimum # of detections to output
    ...
    func evaluate(inputs: [MLMultiArray], outputs: [MLMultiArray]) throws {
        ...
        for classId in classIdSet {
            ...
            let nmsResults = nonMaxSupression(boxes: filteredRois, indices: indicesOfClass, iouThreshold: self.nmsIOUThreshold, max: self.maxDetections)
            nmsBoxIds.append(contentsOf: nmsResults)
        }
        
        // Keep the top NOut
        let resultIndices: [Int] = { () -> [Int] in
            ...
            let sortedZippedIds = zippedIdsAndScores.sorted(by: { (a, b) -> Bool in
                return a.1 > b.1
            })
        ...
        // Layout output is [NOut, (y1,x2,y2,x2,class_id,score)]
...
func indicesOfRoisWithHighScores(scores: UnsafeMutablePointer<Float>,
    ...
    vDSP_vclipc(scores, 1, UnsafeMutablePointer<Float>(&lowThreshold), UnsafeMutablePointer<Float>(&highThreshold), temporaryBuffer, 1, count, UnsafeMutablePointer<UInt>(&lowCount), UnsafeMutablePointer<UInt>(&highCount))
    ...
    // Compress the ramp
    vDSP_vcmprs(temporaryBuffer, 1, scores, 1, UnsafeMutablePointer<Float>(&indices), 1, count)
