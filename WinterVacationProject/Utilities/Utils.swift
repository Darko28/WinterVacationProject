...
extension UnsafePointer where Pointee == Float {
    ...
    func indexed(indices: [Float]) -> [Float] {
        ...
        var indices = indices
        vDSP_vindex(self, UnsafeMutablePointer<Float>(&indices), 1, UnsafeMutablePointer<Float>(&results), 1, vDSP_Length(indices.count))
...
extension Array where Element == Float {
    ...
    func sortedIndices(ascending: Bool) -> [UInt] {
        ...
        vDSP_vsorti(UnsafeMutablePointer<Float>(&array), UnsafeMutablePointer<vDSP_Length>(&indices), nil, vDSP_Length(array.count), ascending ? 1 : -1)
...
extension MLMultiArray {
    ...
    func padTailWithZeros(startIndex: Int, count: Int) {
        ...
        let paddingBufferPointer = UnsafeMutablePointer(&paddingBuffer)
...
func broadcastedIndices(indices: [Float], toElementLength elementLength: Int) -> [Float] {
    ...
    for i in 0..<Int(elementLength) {
        cblas_scopy(Int32(indices.count), UnsafeMutablePointer<Float>(&indices), 1, UnsafeMutablePointer<Float>(&resultIndices).advanced(by: i), Int32(elementLength))
    }
    
    var multiplicationScalar = Float(elementLength)
    vDSP_vsmul(UnsafeMutablePointer<Float>(&resultIndices), 1, UnsafeMutablePointer<Float>(&multiplicationScalar), UnsafeMutablePointer<Float>(&resultIndices), 1, vDSP_Length(resultCount))
    ...
    for i in 1..<Int(elementLength) {
        ...
        vDSP_vsadd(UnsafeMutablePointer<Float>(&resultIndices).advanced(by: i), vDSP_Stride(elementLength), shiftPointer, UnsafeMutablePointer<Float>(&resultIndices).advanced(by: i), vDSP_Stride(elementLength), vDSP_Length(indices.count))
    }
...
func computeIndices(fromIndicesPointer: UnsafeMutablePointer<Float>, toIndicesPointer: UnsafeMutablePointer<Float>,
                    elementLength: UInt, elementCount: UInt) {
...
func elementWiseMultiply(matrixPointer: UnsafeMutablePointer<Float>, vectorPointer: UnsafeMutablePointer<Float>,
...
// nonMaxSupression
// This is also a bottleneck, find a way to vectorize some parts of it. Also,is it worth to compute these without having to transform the floats into CG structures?
func nonMaxSupression(boxes: [Float], indices: [Int], iouThreshold: Float, max: Int) -> [Int] {
    ...
    for index in indices {
        ...
        if (shouldSelect) {
            // Does the current box overlap one of the selected anchors more than the given threshold amount?
            // Then it's too similar, so don't keep it.
            for j in selected {
        ...
        // This bounding box did not overlap too much with any previously selected bounding box,
        // so we'll keep it.
        if shouldSelect {
